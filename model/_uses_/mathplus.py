...
def square_root(n:int, do_round=True):
    return round(n ** (1/2)) if do_round else n ** (1/2)
...
def is_prime(n):
    return all(n % i != 0 for i in range(2, n)) if n not in [1, 0] else False
...
def get_percentage(total=100, nb=100, do_round=True):
    return round((100*nb)/total) if do_round else (100*nb)/total
...
class Function:
    ...
    def get_image_for(self, x, do_round=True):
        ...
        return round(eval(split)) if do_round else eval(split)
...
def decrypt(string: str, key=None, caracters="*=(£é_ç$+}", alphabet="abcdefghijklmnopqrstuvwxyz1234567890,'.!:? "):
    ...
    go_string = []
    go_string = [string[i] for i in range(len(string)) if is_even(i)]
